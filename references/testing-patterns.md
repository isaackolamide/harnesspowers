...
1. **Red**: Write a failing unit or integration test before implementing the logic.
2. **Green**: Write the minimum amount of production code required to make the test pass.
3. **Refactor**: Clean up the code (remove duplication, improve design/naming) while keeping tests passing.
...
### Test Data & Mock Builders
Use the **Builder Pattern** to construct entities or mock outputs for tests. This keeps tests resilient to changes in object/class constructor signatures.
...
// In the test:
const adminUser = new UserBuilder().withAdminRole().build();
```
