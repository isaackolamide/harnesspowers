...
1. **CLASSIFY**: Match the task title, scope description, and filenames against keywords to identify required instructions:
   - `frontend`, `UI`, `component`, `page`, `layout`, `design`, `style` → Instruct implementer to use `agent-skills:frontend-ui-engineering`
   - `API`, `endpoint`, `schema`, `interface`, `contract`, `route` → Instruct implementer to use `agent-skills:api-and-interface-design`. (If making a significant architectural design choice, write an ADR at the controller level using `agent-skills:documentation-and-adrs` first, then pass the ADR path).
...
3. **DISPATCH**: Run `superpowers:subagent-driven-development` for the task.
